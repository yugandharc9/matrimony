...
import { createChatRequest, declineChatRequest, getBioDataOfProfile, saveProfile } from '../../services/apiService';
...
const ProfileCard = ({ name, lastName, age, current_city, occupation, degree, height_ft, userID, profileID, aboutme, pics, work, totalpoints, points, context }) => {
  ...
  const handleChatReqConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
      try {
  ...
  const handleSaveProfileConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
      console.log(`Action confirmed for user ID: ${userID}`);
      // Additional logic after confirmation
      try {
  ...
  const handleAcceptInterestReqConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
  ...
  const handleRejectInterestConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
      console.log(`Action confirmed for user ID: ${userID}`);
      try {
        const resp = null;
  ...
  const handleAcceptChatReqConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
  ...
  const handleRejectChatReqConfirm = async ({ actionConfirmed, userID, profileID }) => {
    if (actionConfirmed) {
      console.log(`Action confirmed for user ID: ${profileID}`);
      // Additional logic after confirmation
      try {
  ...
  return (
    <div className="max-w-sm mt-2 rounded-lg overflow-hidden shadow-lg border border-[#492533] bg-[#FEF5EC] p-4 transform transition duration-300 hover:scale-105 hover:shadow-xl">
      ...
      <div className="py-6 px-3 text-center">
        ...
        <ConfirmDialog ref={superLikeRef} profileID={profileID} userID={userID} onConfirm={handleSuperLikeConfirm} message={`Do you want to send interest ${name}? If ${name} is interested you will be a match`} />
        ...
        <div className='inline space-x-6'>
          ...
          <ConfirmDialog ref={saveProfileRef} profileID={profileID} userID={userID} onConfirm={handleSaveProfileConfirm} message={`Do you want to save ${name}'s profile?`} />
          < VisibilityIcon sx={{
            color: "",
            fontSize: "50px",
          }}
            //onClick={showProfile(Buffer.from(profileID.toString()).toString("base64"))}
            onClick={() => { showProfile(btoa(profileID))}}
          />
          ...
          {context == "chatRequests" && (<>
            ...
            <ConfirmDialog ref={acceptChatReqRef} profileID={profileID} userID={userID} onConfirm={handleAcceptChatReqConfirm} message={`Do you want accept ${name}'s chat request?`} />
            <CloseIcon sx={{
              transform: 'scale(1)',
              fontSize: "50px",
              transition: 'transform 0.3s ease-in-out',
            ...
            <ConfirmDialog ref={rejectChatReqRef} profileID={profileID} userID={userID} onConfirm={handleRejectChatReqConfirm} message={`Do you want decline ${name}'s chat request?`} />
          ...
          {context == "interests" && (<>
            ...
            <ConfirmDialog ref={acceptInterestReqRef} profileID={profileID} userID={userID} onConfirm={handleAcceptInterestReqConfirm} message={`Do you want show interest in ${name}?`} />
            <CloseIcon sx={{
              transform: 'scale(1)',
              fontSize: "50px",
              transition: 'transform 0.3s ease-in-out',
            ...
            <ConfirmDialog ref={rejectInterestReqRef} profileID={profileID} userID={userID} onConfirm={handleRejectInterestConfirm} message={`Do you want decline interest by ${name}?`} />
          ...
          {context == "profiles" && (<><ChatIcon sx={{
            ...
            < DownloadIcon sx={{
              ...
              onClick={() => { handleDownload(name, lastName, userID) }}
            /> </>)}
